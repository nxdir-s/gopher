...
CPU a generate spends**, run once per `.go` artifact. Formatting at runtime is
not optional overhead — it is the safety net that keeps user override
...
- **Single-file generates sit at a floor.** `adapter -kind aws` is one ~12.6KB
  ...
  parse-and-print cannot be split, so `BenchmarkGenerateCold/adapter_aws`
...
- **Multi-file generates parallelize.** `setup` is 14 refs and 10 formats;
  ...
  is about a third of the total — which is why `MaxRenderFan` is 3 and not the
...
per render because every template string in one invocation is distinct — the
documented reason there is no parse cache), the registry building specs, the
config load's upward walk and file reads, and the process itself.
...
everything gopher controls. That is not a reason to stop measuring — it is the
...
- **Thermal state contaminates whole runs.** A `-count 10` full-suite run
  ...
  benchmarks whose code did not change. Let the machine sit idle a few minutes
...
results land by ref index so artifact order and goldens cannot change, the
...
and clobbered; now it is a hard error.
